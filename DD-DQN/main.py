...
class Qnetwork():
    def __init__(self, h_size, lr):
        ...
        self.actions_onehot = tf.one_hot(
            self.actions, env.actions, dtype=tf.float32)

        self.Q = tf.reduce_sum(
            tf.multiply(self.Qout, self.actions_onehot), axis=1)
...
path = "./models"  # The path to save our model to.
...
mainQN = Qnetwork(h_size, lr)
targetQN = Qnetwork(h_size, lr)
